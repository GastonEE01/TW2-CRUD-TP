...
export class CarritoService {
    ...
    async obtenerCarrito(idUsuario: number) {
        ...
        if (carrito && !carrito.CarritoProducto) {
    ...
    async agregarProducto(idUsuario: number, idProducto: number, cantidad: number = 1) {
        ...
        const resultado = await this.carritoRepository.agregarProductoAlCarrito(carrito.id, idProducto, cantidad);
    ...
    async limpiarCarrito(idUsuario: number) {
        ...
        if (carrito) {
            return await this.carritoRepository.limpiarCarrito(carrito.id);
        }
        return null;
    }
}